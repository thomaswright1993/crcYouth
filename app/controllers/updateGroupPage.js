var clickMarker;
var map;
angular.module('indexApp').controller('UpdateGroupCtrl', function($scope, $http, $routeParams) {
    $scope.group = [];
    ...
    google.maps.event.addListener(map, 'click', function(event) {
        clickMarker.position = event.latLng;
    ...
    $http.get('/getGroups').success(function (groupResults) {
        for(var i = 0; i < groupResults.length; i++) {
            ...
            if($routeParams.groupID === groupResults[i].id){
                marker.icon = '/images/newPin.png';
                clickMarker = marker;
            }
